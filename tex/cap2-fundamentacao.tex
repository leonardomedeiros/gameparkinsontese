...
Optou-se utilizar a ~\cite{SVM} devido a sua capacidade de generalização nos problemas de classificação de dados~\cite{vapnik95,xusvm2009}, sua robustez nos resultados~\cite{xusvm2009} e sua performance com baixa complexidade computacional em comparação a outras abordagens como Redes Neurais~\cite{comprnasvm2007}.
...
A partir desse método de separação de dados lineares, é que a~\ac{svm} foi aplicada para classificar indivíduos diagnosticados com~\ac{dp} ante indivíduos sem o diagnóstico estabelecido. Para corroborar com a nossa escolha de SVM, encontrou-se trabalhos de classificação de indivíduos com parkinson utilizando a mesma abordagem~\cite{bradmonitor2015,svmparkinson2010,patel_monitoring_2009}. No entanto, o que diferencia a presente abordagem das demais, é justamente quais serão os dados processados a serem classificados. Como explicamos anteriormente nos objetivos do trabalho (Seção~\ref{section:objetivos}), está sendo avaliado a utilização de jogos eletrônicos responsáveis por induzir a execução de movimentos~\cite{quantitativeparkinson2011} para o acompanhamento motor do~\ac{dp}. Logo, na Seção~\ref{sec:processador_bio}, será apresentado o Processador de Dados Biomecânicos responsável por extrair as estas características do movimento~\cite{mcginnis2013biomechanics}para obter a classificação exposta na Seção~\ref{sec:resultado_svm}.
